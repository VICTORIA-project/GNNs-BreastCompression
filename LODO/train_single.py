...
# Training
def train(dataset, final_test, writer, dataset_name, config_selected):
    data_size = len(dataset)
    print(dataset_name)
    print('Dataset Size: ', data_size)
    print('Dataset Size: ', len(final_test))

    # ------------------------------------------------------------------------------------------------------------------
    # if dataset_name == 'dataset_1':
    print(dataset_name + ' is being used.')

    loader = DataLoader(dataset[:int(data_size * 0.8)], batch_size=8, shuffle=True)
    validation_loader = DataLoader(dataset[int(data_size * 0.8):], batch_size=1, shuffle=False)
    test_loader = DataLoader(final_test, batch_size=1, shuffle=False) 
    # ------------------------------------------------------------------------------------------------------------------
...
def reproduce(dataset_name, dataset, final_test, writer, config_selected, save, mean_mag_results, max_error_results):
    data_size = len(dataset)
    # -----------------------------------------------------------------------------------------------
    if mean_mag_results == 1 and max_error_results == 0:
        validation_loader = DataLoader(dataset[int(data_size * 0.8):], batch_size=8, shuffle=False)
        test_loader = DataLoader(final_test, batch_size=8, shuffle=False)
    else:
        validation_loader = DataLoader(dataset[int(data_size * 0.8):], batch_size=1, shuffle=False)
        test_loader = DataLoader(final_test, batch_size=1, shuffle=False)
    # ------------------------------------------------------------------------------------------------------------------
    ...
    if max_error_results == 1:
        ...
        print("Average running time: {:.4f}".format(mean_time))
